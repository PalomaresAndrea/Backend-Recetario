...
export async function createRecipe(req, res) {
  ...
  const r = await Recipe.create({ ...req.body, createdBy: req.user._id });
...
export async function listRecipes(req, res) {
  ...
  const items = await Recipe.find(filter).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean();
  res.json({ page, limit, total, items });
}

export async function getFeatured(_req, res) {
  const items = await Recipe.find({ published: true }).sort({ likes: -1, createdAt: -1 }).limit(10).lean();
  res.json(items);
}
